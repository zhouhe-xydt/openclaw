...
export function renderAgentTools(params: {
  ...
  const profileSource = agentTools.profile
    ? t("agents.tools.agentOverride")
    : globalTools.profile
      ? t("agents.tools.globalDefault")
      : t("agents.tools.default");
    ...
    <section class="card">
      ...
      <div class="agent-tools-meta" style="margin-top: 16px;">
        ...
        ${
          params.configDirty
            ? html`
                <div class="agent-kv">
                  <div class="label">${t("cron.jobState.status")}</div>
                        ...
                        <div>
                          <div class="agent-tool-title mono">
                            ...
                            ${
                              isOptional
                                ? html`
                                    <span class="mono" style="margin-left: 6px; opacity: 0.8">${t("agents.tools.optional")}</span>
                                  `
                                : nothing
                            }
                          </div>
      ...
      ${
        params.toolsCatalogLoading
          ? html`
              <div class="card-sub" style="margin-top: 10px">${t("agents.tools.refreshing")}</div>
            `
          : nothing
      }
    </section>
  ...
  return html`
    <section class="card">
      ...
      ${
        usingAllowlist
          ? html`
              <div class="callout info" style="margin-top: 12px">${t("agents.skills.customAllowlist")}</div>
            `
          : html`
              <div class="callout info" style="margin-top: 12px">
                ${t("agents.skills.allEnabled")}
              </div>
            `
      }
      ${
  ...
  return html`
    <div class="list-item agent-skill-row">
      <div class="list-main">
        ...
        ${
          missing.length > 0
            ? html`<div class="muted" style="margin-top: 6px;">${t("agents.skills.missing")}: ${missing.join(", ")}</div>`
            : nothing
        }
        ${
          reasons.length > 0
            ? html`<div class="muted" style="margin-top: 6px;">${t("agents.skills.reason")}: ${reasons.join(", ")}</div>`
            : nothing
        }
      </div>
      <div class="list-meta">
